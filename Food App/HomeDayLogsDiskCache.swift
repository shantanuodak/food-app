...
/// Disk-backed stale-while-revalidate cache for home hydration day logs.
/// Mirrors `HomeDayLogsDiskCache` so water survives cold starts and paints
/// instantly on day-switch, exactly like food logs.
enum HomeHydrationDayLogsDiskCache {
    private static let keyPrefix = "app.hydrationdaylogs.cache.v1."

    static func persist(_ response: HydrationDayLogsResponse, date: String, defaults: UserDefaults = .standard) {
        guard let data = try? JSONEncoder().encode(response) else { return }
        defaults.set(data, forKey: keyPrefix + date)
    }

    static func load(date: String, defaults: UserDefaults = .standard) -> HydrationDayLogsResponse? {
        guard let data = defaults.data(forKey: keyPrefix + date) else { return nil }
        return try? JSONDecoder().decode(HydrationDayLogsResponse.self, from: data)
    }
