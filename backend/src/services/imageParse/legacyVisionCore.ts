...
export type ImageParseUsageEvent = {
  feature:
    | 'parse_image_primary'
...
function uniqueCaptionNames(names: string[]): string[] {
...
function captionNameFromRecord(record: Record<string, unknown>): string {
  return asText(getFirst(record, ['name', 'foodName', 'food', 'item', 'label', 'description', 'foodDescription']));
}

function captionNamesFromArray(value: unknown): string[] {
  if (!Array.isArray(value)) {
    return [];
  }
  return value.flatMap((entry) => {
    ...
    const name = record ? captionNameFromRecord(record) : '';
    return name ? [name] : [];
  });
}

function captionFromLooseStructuredText(value: string): string | null {
  const names: string[] = [];
  const namePattern = /"(?:name|foodName|food|item|label)"\s*:\s*"([^"]{2,80})"/gi;
  let match: RegExpExecArray | null;
  while ((match = namePattern.exec(value)) !== null) {
    names.push(match[1]);
  }
  const unique = uniqueCaptionNames(names);
  return unique.length ? unique.join(', ').slice(0, 500) : null;
}

function captionFromStructuredPayload(parsed: unknown): string | null {
  if (typeof parsed === 'string') {
    return parsed.trim().slice(0, 500) || null;
  }

  if (Array.isArray(parsed)) {
    const names = uniqueCaptionNames(captionNamesFromArray(parsed));
    return names.length ? names.join(', ').slice(0, 500) : null;
  }
  ...
  const directCaption = asText(getFirst(record, ['caption', 'description', 'meal', 'text', 'summary', 'extractedText']));
  if (directCaption) {
    return directCaption.slice(0, 500);
  }

  const names = uniqueCaptionNames([
    ...captionNamesFromArray(getFirst(record, ['visibleComponents', 'components', 'detectedComponents'])),
    ...captionNamesFromArray(getFirst(record, ['items', 'foods', 'foodItems', 'detectedFoods', 'detected_foods', 'results']))
  ]);
  return names.length ? names.join(', ').slice(0, 500) : null;
}
...
function parseCaptionPayload(payloadText: string): string | null {
  ...
  if (jsonCandidate) {
    try {
      ...
      const caption = captionSchema.safeParse(parsed);
      if (caption.success) {
        return caption.data.caption;
      }
      const structuredCaption = captionFromStructuredPayload(parsed);
      if (structuredCaption) {
        return structuredCaption;
      }
    } catch {
      const looseStructuredCaption = captionFromLooseStructuredText(jsonCandidate);
      if (looseStructuredCaption) {
        return looseStructuredCaption;
      }
    }
  }
  ...
  try {
    ...
    const looseStructuredCaption = captionFromLooseStructuredText(trimmed);
    if (looseStructuredCaption) {
      return looseStructuredCaption;
    }
...
async function runImageCaptionFallback(
  ...
  const response = await generateGeminiMultimodalText({
    model,
    temperature: 0.1,
    maxOutputTokens: mode === 'inventory' ? 320 : 120,
    timeoutMs,
    maxAttempts: 1,
    parts: [
...
function userCaptionContext(contextNote?: string): string {
  const note = trimSafe(contextNote);
  if (!note) {
    return '';
  }

  const generatedContextMarker = note.search(/\n\nCuisine router:/i);
  if (generatedContextMarker > 0) {
    return note.slice(0, generatedContextMarker).trim();
  }

  if (
    /^Cuisine router:/i.test(note) ||
    note.includes('Apply the cuisine guidance below') ||
    note.includes('You are a nutrition parser for a food-logging app')
  ) {
    return '';
  }
...
function partialCoverageFromSparseCaption(
  caption: string,
  result: ParseResult,
  guardReason: string
): ImageParseCoverage {
  const base = coverageFromCaptionInventory(caption, result);
  const visibleComponentCount = Math.max(base.visibleComponentCount, result.items.length + 1, 3);
  const parsedItemCount = result.items.length;
  const score = visibleComponentCount > 0 ? round(Math.min(1, parsedItemCount / visibleComponentCount), 2) : 0;
  ...
  return {
    ...
    partial: true,
    warnings: [
      'This is a partial photo estimate. Please review the detected foods because the detailed image pass timed out.',
      guardReason.replace(/_/g, ' ')
    ]
  };
}
...
const captionEstimateLibrary: CaptionEstimate[] = [
  ...
  { name: 'Green vegetable stir-fry', aliases: ['green vegetable stir-fry', 'green vegetable stir fry', 'stir-fried green vegetable', 'stir fried green vegetable', 'cooked leafy greens', 'leafy greens'], quantity: 1, unit: 'side', grams: 100, calories: 110, protein: 4, carbs: 12, fat: 5 },
  { name: 'Chocolate wafer bar', aliases: ['chocolate wafer bar', 'wafer bar', 'chocolate coated wafer bar'], quantity: 1, unit: 'bar', grams: 42, calories: 210, protein: 2, carbs: 25, fat: 11 },
  { name: 'Chocolate bar', aliases: ['chocolate bar', 'candy bar'], quantity: 1, unit: 'bar', grams: 45, calories: 230, protein: 3, carbs: 26, fat: 13 },
  { name: 'Rusk', aliases: ['rusk', 'toast rusk', 'tea rusk'], quantity: 2, unit: 'pieces', grams: 24, calories: 100, protein: 2, carbs: 18, fat: 2.5 },
  { name: 'Crackers', aliases: ['cracker', 'crackers', 'saltine crackers'], quantity: 4, unit: 'crackers', grams: 16, calories: 70, protein: 1.5, carbs: 12, fat: 2 },
  { name: 'Ajwain tea', aliases: ['ajwain tea', 'carom seed tea'], quantity: 1, unit: 'cup', grams: 240, calories: 2, protein: 0, carbs: 0.5, fat: 0 },
  ...
  { name: 'Palak paneer', aliases: ['palak paneer', 'saag paneer'], quantity: 1, unit: 'serving', grams: 180, calories: 320, protein: 15, carbs: 16, fat: 23 },
  { name: 'Papad', aliases: ['papad', 'papadum', 'appalam'], quantity: 1, unit: 'piece', grams: 15, calories: 55, protein: 2, carbs: 10, fat: 1 },
  ...
  { name: 'Vada pav', aliases: ['vada pav'], quantity: 1, unit: 'piece', grams: 180, calories: 360, protein: 9, carbs: 50, fat: 13 },
  ...
  { name: 'Gajar halwa', aliases: ['gajar halwa', 'carrot halwa'], quantity: 1, unit: 'bowl', grams: 180, calories: 430, protein: 7, carbs: 58, fat: 19 },
  { name: 'Cashews', aliases: ['cashews', 'cashew'], quantity: 1, unit: 'small garnish', grams: 15, calories: 85, protein: 2.7, carbs: 4.5, fat: 6.6 },
  { name: 'Almonds', aliases: ['almonds', 'almond'], quantity: 1, unit: 'small garnish', grams: 12, calories: 70, protein: 2.5, carbs: 2.6, fat: 6 },
  { name: 'Jalebi', aliases: ['jalebi'], quantity: 2, unit: 'pieces', grams: 110, calories: 420, protein: 4, carbs: 72, fat: 14 },
  { name: 'Rabri', aliases: ['rabri', 'rabdi', 'sweet cream sauce'], quantity: 0.5, unit: 'cup', grams: 120, calories: 310, protein: 8, carbs: 32, fat: 17 },
  { name: 'Pistachios', aliases: ['pistachios', 'pistachio'], quantity: 1, unit: 'small garnish', grams: 8, calories: 45, protein: 1.6, carbs: 2.2, fat: 3.6 },
  { name: 'Gulab jamun', aliases: ['gulab jamun'], quantity: 1, unit: 'piece', grams: 50, calories: 150, protein: 2, carbs: 24, fat: 6 },
  { name: 'Ladoo', aliases: ['ladoo', 'laddu', 'sweet balls'], quantity: 3, unit: 'pieces', grams: 120, calories: 480, protein: 8, carbs: 62, fat: 22 },
  ...
  { name: 'Fried chicken sandwich', aliases: ['fried chicken sandwich', 'chicken sandwich'], quantity: 1, unit: 'sandwich', grams: 260, calories: 650, protein: 32, carbs: 52, fat: 34 },
  { name: 'Side salad', aliases: ['side salad', 'salad', 'mixed greens'], quantity: 1, unit: 'bowl', grams: 150, calories: 90, protein: 3, carbs: 12, fat: 4 },
  { name: 'Stout beer', aliases: ['stout beer', 'stout'], quantity: 1, unit: 'pint', grams: 473, calories: 210, protein: 2, carbs: 18, fat: 0 },
  { name: 'Cocktail', aliases: ['red cocktail', 'cocktail'], quantity: 1, unit: 'glass', grams: 180, calories: 180, protein: 0, carbs: 20, fat: 0 },
  { name: 'Bread', aliases: ['bread'], quantity: 1, unit: 'piece', grams: 40, calories: 110, protein: 4, carbs: 20, fat: 1.5 },
...
function expandCaptionInventoryWithVisualHeuristics(caption: string): { caption: string; added: string[] } {
  const key = captionSegmentKey(caption);
  const added: string[] = [];
  const hasAny = (pattern: RegExp) => pattern.test(key);

  if (
    hasAny(/\bdal\b/) &&
    hasAny(/\bgreen chutney\b/) &&
    hasAny(/\bpotato sabzi\b/) &&
    hasAny(/\bonion\b/) &&
    !hasAny(/\b(baati|bati|roti|chapati|paratha|naan|rice)\b/)
  ) {
    added.push('Baati');
  }

  if (
    hasAny(/\b(paratha|roti|chapati)\b/) &&
    hasAny(/\bwhite rice\b|\brice\b/) &&
    hasAny(/\bdal\b/) &&
    hasAny(/\b(chole|palak paneer|saag paneer)\b/) &&
    !hasAny(/\b(papad|papadum|appalam)\b/)
  ) {
    added.push('Papad');
  }
  ...
  return { caption: mergeCaptionTexts([caption, added.join(', ')]), added };
}
...
async function recoverWithV2CaptionEnsemble(
  ...
  const model = config.aiImageCaptionModel.trim() || config.aiImageInventoryModel.trim() || config.geminiFlashModel;
  ...
  const allowSparsePartial = options.allowSparsePartial ?? false;
  const timeoutMs = options.captionTimeoutMs ?? Math.min(Math.max(config.aiImageFastTimeoutMs, 2_400), 3_000);
  ...
  const contextCaption = userCaptionContext(image.contextNote);
  if (
    contextCaption &&
  ...
  if (initialGuardReason && !singleProductCaptionResult(mergedCaption)) {
    const probeTimeoutMs = options.probeTimeoutMs ?? Math.min(Math.max(config.aiImageFastTimeoutMs, 2_200), 3_000);
  ...
  const visuallyExpanded = expandCaptionInventoryWithVisualHeuristics(mergedCaption);
  if (visuallyExpanded.added.length > 0) {
    mergedCaption = visuallyExpanded.caption;
    image.debugEvents?.push({
      stage: 'image_caption_fast_v2',
      ok: true,
      model: 'heuristic',
      reason: 'expanded_common_visual_inventory',
      ms: 0,
      items: captionFoodSegmentCount(mergedCaption),
      caption: mergedCaption.slice(0, 180)
    });
  ...
  if (heuristic) {
    ...
    if (guardReason) {
      ...
      if (!allowStructuredRescue) {
        if (!allowSparsePartial) {
          return null;
        }

        const sparseCoverage = partialCoverageFromSparseCaption(mergedCaption, heuristic, guardReason);
        const sparseResult = postProcessFoodImageResult(
          imageSafeResult(normalizeParseResultContract(
            {
              ...heuristic,
              confidence: Math.min(heuristic.confidence, 0.68),
              assumptions: [
                ...heuristic.assumptions,
                'Returned a partial estimate because the fast caption was sparse and the detailed image pass was still needed.'
              ]
            },
            'gemini'
          )),
          {
            ...
            imageType: sparseCoverage.imageType,
            visibleComponents: sparseCoverage.visibleComponents
          }
        );
        ...
        image.debugEvents?.push({
          ...
          ok: true,
          model: 'heuristic',
          reason: `${guardReason}_partial_reviewable_result`,
          ms: 0,
          confidence: sparseResult.confidence,
          items: sparseResult.items.length,
          caption: mergedCaption.slice(0, 180)
        });
  ...
  return parseCaptionToImageResult(
    ...
    'v2',
    undefined,
    options.captionTextTimeoutMs ?? Math.min(Math.max(config.aiImageFastTimeoutMs, 2_500), 3_000)
  );
}
...
async function recoverWithStructuredRescue(
  ...
  const rescueModels = Array.from(
    new Set([config.aiImageInventoryModel.trim(), config.aiImageFallbackModel.trim()].filter(Boolean))
  );
  const startedAt = process.hrtime.bigint();
...
function shouldDeferCaptionResultForRescue(candidate: ImageParseServiceResult | null): boolean {
  if (!candidate?.coverage?.partial) {
    return false;
  }
  return candidate.result.confidence < config.aiImageConfidenceMin;
}
  ...
  const visionImage = await prepareImageForVision(image);
  {
    ...
    const captionTask = trackAsyncTask(
      recoverWithV2CaptionEnsemble(visionImage, captionUsageEvents, {
        ...
        allowSparsePartial: true,
        captionTimeoutMs: Math.min(Math.max(config.aiImageFastTimeoutMs, 2_400), 3_000),
        probeTimeoutMs: Math.min(Math.max(config.aiImageFastTimeoutMs, 2_200), 3_000),
        captionTextTimeoutMs: Math.min(Math.max(config.aiImageFastTimeoutMs, 2_500), 3_000)
      }),
      (error) => {
        visionImage.debugEvents?.push({
          stage: 'image_orchestrator_v2',
    ...
    if (captionEnsemble && resultHasPositiveNutrition(captionEnsemble.result) && !shouldDeferCaptionResultForRescue(captionEnsemble)) {
    ...
    if (captionEnsemble && resultHasPositiveNutrition(captionEnsemble.result)) {
      visionImage.debugEvents?.push({
        ...
        ms: elapsedMs(orchestratorStartedAt),
        items: captionEnsemble.result.items.length,
        caption: captionEnsemble.coverage
          ? `visible=${captionEnsemble.coverage.visibleComponentCount} · coverage=${captionEnsemble.coverage.score}`
          : undefined
      });
      return {
        ...captionEnsemble,
